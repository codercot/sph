          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key=item.id>
                ...
                <dd changepirce="0" v-for="(iten, index) in item.spuSaleAttrValueList" :key="iten.id"
                  :class="{ active: iten.isChecked == 1 }" @click="paiTa(iten, item.spuSaleAttrValueList)">{{
                    iten.saleAttrValueName }}</dd>
              </dl>

            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="number" @change="changeNum">
...
export default {
  ...
  methods: {
    ...
    async btn() {
      ...
      try {
        // 1.存储成功执行
        await this.$store.dispatch("getGou", { id: this.$route.params.id, num: this.number })
        // 2.路由跳转,传递数量，商品信息由会话存储进行传递
        this.$router.push({ name: "addcartsucess" ,query:{number:this.number}})
        // 商品详情存储到会话中
