<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item, index) in getAdressInfo" :key="item.id">
        <span class="username " :class="{ selected: item.isDefault == 1 }">{{ item.consignee }}</span>
        <p @click="btn(item, getAdressInfo)">
          <span class="s1">{{ item.fullAddress }}</span>
          <span class="s2">电话:{{ item.phoneNum }}</span>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ getUserInfo.fullAddress }}</span>
        收货人：<span>{{ getUserInfo.consignee }}</span>
        <span>{{ getUserInfo.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...mapState({
      getAdressInfo: state => state.Trade.adress,
      orderInfo: state => state.Trade.order
    }),
    // 计算提交订单上面的数据
    getUserInfo() {
      return this.getAdressInfo.find(item => item.isDefault == 1) || {}
    }
  },
  methods: {
    // 选择地址
    ...
    async submitOrder() {
      let data = {
        "consignee": this.getUserInfo.consignee || this.consignee,
        "consigneeTel": this.getUserInfo.phoneNum || this.phoneNum,
        "deliveryAddress": this.getUserInfo.fullAddress || this.fullAddress,
        "paymentWay": "ONLINE",
        "orderComment": this.message,
        "orderDetailList": this.orderInfo.detailArrayList
      }
      let res = await this.$API.reqSubmitOrder(this.orderInfo.tradeNo, data)
      if (res.code == 200) {
